 ...
 */
package org.finos.flowave.bpm.engine.cdi.jsf;
...
public class TaskForm implements Serializable {
  ...
  public void startTaskForm() {
    ...
    if (taskId==null || callbackUrl == null) {
      ...
      // return it anyway but log an info message
      log.log(Level.INFO, "Called startTask method without proper parameter (taskId='"+sanitize(taskId)+"'; callbackUrl='"+sanitize(callbackUrl)+"') even if it seems we are not called by an AJAX Postback. Are you using the camundaTaskForm bean correctly?");
      return;
    }
    // Note that we always run in a conversation
    this.url = callbackUrl;
  ...
  /*
   * This method is used to sanitize the input before logging it, ensuring that no sensitive information is exposed.
   * It removes newline characters, escapes HTML-specific characters, and replaces single and double quotes with safe alternatives.
   */
