...
import static org.finos.flowave.bpm.engine.rest.helper.MockProvider.EXAMPLE_PRIMITIVE_VARIABLE_VALUE;
import static org.finos.flowave.bpm.engine.rest.helper.MockProvider.EXAMPLE_VARIABLE_INSTANCE_NAME;
...
import org.finos.flowave.bpm.engine.externaltask.ExternalTaskQueryTopicBuilder;
import org.finos.flowave.bpm.engine.externaltask.LockedExternalTask;
import org.finos.flowave.bpm.engine.externaltask.UpdateExternalTaskRetriesBuilder;
...
import org.finos.flowave.bpm.engine.impl.externaltask.FetchAndLockBuilderImpl;
import org.finos.flowave.bpm.engine.rest.dto.SortingDto;
import org.finos.flowave.bpm.engine.rest.dto.externaltask.ExternalTaskQueryDto;
...
import org.finos.flowave.bpm.engine.rest.impl.FetchAndLockContextListener;
...
public class ExternalTaskRestServiceInteractionTest extends AbstractRestServiceTest {
  ...
  protected FetchAndLockBuilderImpl fetchAndLockBuilder;
  ...
  public void testFetchAndLockWithSortOrderInvalidCase() {
    ...
    given()
            ...
            .body("message", Matchers.equalTo("Cannot set query sortOrder parameter to value DESC"))
  ...
  public void testFetchAndLockWithEmptySortOrder() {
    ...
    parameters.put("workerId", "aWorkerId");
    parameters.put("usePriority", false);
    parameters.put("sorting", List.of(create("createTime", "")));
    ...
    given()
            ...
            .then().expect().statusCode(Status.BAD_REQUEST.getStatusCode())
            .body("type", Matchers.equalTo(InvalidRequestException.class.getSimpleName()))
            .body("message", Matchers.equalTo("Cannot set query sortOrder parameter to value "))
            .when().post(FETCH_EXTERNAL_TASK_URL);
  ...
  public void testFetchAndLockWithBlankSortOrder() {
    ...
    parameters.put("workerId", "aWorkerId");
    parameters.put("usePriority", false);
    parameters.put("sorting", List.of(create("createTime", sortOrder)));
    ...
    given()
            ...
            .then().expect().statusCode(Status.BAD_REQUEST.getStatusCode())
            .body("type", Matchers.equalTo(InvalidRequestException.class.getSimpleName()))
            .body("message", Matchers.equalTo("Cannot set query sortOrder parameter to value " + sortOrder))
            .when().post(FETCH_EXTERNAL_TASK_URL);
  ...
  protected void executePost(Map<String, Object> parameters) {
    given()
        ...
        .body("[0].variables." + EXAMPLE_VARIABLE_INSTANCE_NAME, notNullValue())
        .body("[0].variables." + EXAMPLE_VARIABLE_INSTANCE_NAME + ".value", equalTo(EXAMPLE_PRIMITIVE_VARIABLE_VALUE.getValue()))
        .body("[0].variables." + EXAMPLE_VARIABLE_INSTANCE_NAME + ".type", equalTo("String"))
