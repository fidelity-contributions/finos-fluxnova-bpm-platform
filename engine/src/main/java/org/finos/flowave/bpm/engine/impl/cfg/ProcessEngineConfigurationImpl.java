 ...
 */
package org.finos.flowave.bpm.engine.impl.cfg;


import static org.finos.flowave.bpm.engine.impl.cmd.HistoryCleanupCmd.MAX_THREADS_NUMBER;
import static org.finos.flowave.bpm.engine.impl.util.EnsureUtil.ensureNotNull;
...
import org.finos.flowave.bpm.dmn.engine.DmnEngine;
import org.finos.flowave.bpm.dmn.engine.DmnEngineConfiguration;
import org.finos.flowave.bpm.dmn.engine.impl.DefaultDmnEngineConfiguration;
import org.finos.flowave.bpm.dmn.engine.impl.spi.el.ElProvider;
import org.finos.flowave.bpm.dmn.feel.impl.scala.function.FeelCustomFunctionProvider;
import org.finos.flowave.bpm.engine.ArtifactFactory;
...
import org.finos.flowave.bpm.engine.FilterService;
...
import org.finos.flowave.bpm.engine.authorization.Groups;
import org.finos.flowave.bpm.engine.authorization.Permission;
import org.finos.flowave.bpm.engine.authorization.Permissions;
import org.finos.flowave.bpm.engine.impl.AuthorizationServiceImpl;
import org.finos.flowave.bpm.engine.impl.DecisionServiceImpl;
import org.finos.flowave.bpm.engine.impl.DefaultArtifactFactory;
import org.finos.flowave.bpm.engine.impl.ExternalTaskServiceImpl;
import org.finos.flowave.bpm.engine.impl.FilterServiceImpl;
import org.finos.flowave.bpm.engine.impl.FormServiceImpl;
import org.finos.flowave.bpm.engine.impl.HistoryServiceImpl;
import org.finos.flowave.bpm.engine.impl.IdentityServiceImpl;
import org.finos.flowave.bpm.engine.impl.ManagementServiceImpl;
import org.finos.flowave.bpm.engine.impl.ModificationBatchJobHandler;
import org.finos.flowave.bpm.engine.impl.OptimizeService;
import org.finos.flowave.bpm.engine.impl.PriorityProvider;
import org.finos.flowave.bpm.engine.impl.ProcessEngineImpl;
import org.finos.flowave.bpm.engine.impl.ProcessEngineLogger;
import org.finos.flowave.bpm.engine.impl.RepositoryServiceImpl;
import org.finos.flowave.bpm.engine.impl.RestartProcessInstancesJobHandler;
import org.finos.flowave.bpm.engine.impl.RuntimeServiceImpl;
import org.finos.flowave.bpm.engine.impl.ServiceImpl;
import org.finos.flowave.bpm.engine.impl.TaskServiceImpl;
import org.finos.flowave.bpm.engine.impl.application.ProcessApplicationManager;
import org.finos.flowave.bpm.engine.impl.batch.BatchJobHandler;
import org.finos.flowave.bpm.engine.impl.batch.BatchMonitorJobHandler;
import org.finos.flowave.bpm.engine.impl.batch.BatchSeedJobHandler;
import org.finos.flowave.bpm.engine.impl.batch.deletion.DeleteHistoricProcessInstancesJobHandler;
import org.finos.flowave.bpm.engine.impl.batch.deletion.DeleteProcessInstancesJobHandler;
import org.finos.flowave.bpm.engine.impl.batch.externaltask.SetExternalTaskRetriesJobHandler;
import org.finos.flowave.bpm.engine.impl.batch.job.SetJobRetriesJobHandler;
import org.finos.flowave.bpm.engine.impl.batch.message.MessageCorrelationBatchJobHandler;
import org.finos.flowave.bpm.engine.impl.batch.removaltime.BatchSetRemovalTimeJobHandler;
import org.finos.flowave.bpm.engine.impl.batch.removaltime.DecisionSetRemovalTimeJobHandler;
import org.finos.flowave.bpm.engine.impl.batch.removaltime.ProcessSetRemovalTimeJobHandler;
import org.finos.flowave.bpm.engine.impl.batch.update.UpdateProcessInstancesSuspendStateJobHandler;
import org.finos.flowave.bpm.engine.impl.batch.variables.BatchSetVariablesHandler;
import org.finos.flowave.bpm.engine.impl.bpmn.behavior.ExternalTaskActivityBehavior;
import org.finos.flowave.bpm.engine.impl.bpmn.deployer.BpmnDeployer;
import org.finos.flowave.bpm.engine.impl.bpmn.parser.BpmnParseListener;
import org.finos.flowave.bpm.engine.impl.bpmn.parser.BpmnParser;
import org.finos.flowave.bpm.engine.impl.bpmn.parser.DefaultFailedJobParseListener;
import org.finos.flowave.bpm.engine.impl.calendar.BusinessCalendarManager;
import org.finos.flowave.bpm.engine.impl.calendar.CycleBusinessCalendar;
import org.finos.flowave.bpm.engine.impl.calendar.DueDateBusinessCalendar;
import org.finos.flowave.bpm.engine.impl.calendar.DurationBusinessCalendar;
import org.finos.flowave.bpm.engine.impl.calendar.MapBusinessCalendarManager;
import org.finos.flowave.bpm.engine.impl.cfg.auth.AuthorizationCommandChecker;
import org.finos.flowave.bpm.engine.impl.cfg.auth.DefaultAuthorizationProvider;
import org.finos.flowave.bpm.engine.impl.cfg.auth.DefaultPermissionProvider;
import org.finos.flowave.bpm.engine.impl.cfg.auth.PermissionProvider;
import org.finos.flowave.bpm.engine.impl.cfg.auth.ResourceAuthorizationProvider;
import org.finos.flowave.bpm.engine.impl.cfg.multitenancy.TenantCommandChecker;
import org.finos.flowave.bpm.engine.impl.cfg.multitenancy.TenantIdProvider;
import org.finos.flowave.bpm.engine.impl.cfg.standalone.StandaloneTransactionContextFactory;
import org.finos.flowave.bpm.engine.impl.cmd.HistoryCleanupCmd;
import org.finos.flowave.bpm.engine.impl.cmmn.CaseServiceImpl;
import org.finos.flowave.bpm.engine.impl.cmmn.deployer.CmmnDeployer;
import org.finos.flowave.bpm.engine.impl.cmmn.entity.repository.CaseDefinitionManager;
import org.finos.flowave.bpm.engine.impl.cmmn.entity.runtime.CaseExecutionManager;
import org.finos.flowave.bpm.engine.impl.cmmn.entity.runtime.CaseSentryPartManager;
import org.finos.flowave.bpm.engine.impl.cmmn.handler.DefaultCmmnElementHandlerRegistry;
import org.finos.flowave.bpm.engine.impl.cmmn.transformer.CmmnTransformFactory;
import org.finos.flowave.bpm.engine.impl.cmmn.transformer.CmmnTransformListener;
import org.finos.flowave.bpm.engine.impl.cmmn.transformer.CmmnTransformer;
import org.finos.flowave.bpm.engine.impl.cmmn.transformer.DefaultCmmnTransformFactory;
import org.finos.flowave.bpm.engine.impl.db.DbIdGenerator;
import org.finos.flowave.bpm.engine.impl.db.entitymanager.DbEntityManagerFactory;
import org.finos.flowave.bpm.engine.impl.db.entitymanager.cache.DbEntityCacheKeyMapping;
import org.finos.flowave.bpm.engine.impl.db.sql.DbSqlPersistenceProviderFactory;
import org.finos.flowave.bpm.engine.impl.db.sql.DbSqlSessionFactory;
import org.finos.flowave.bpm.engine.impl.delegate.DefaultDelegateInterceptor;
import org.finos.flowave.bpm.engine.impl.diagnostics.DiagnosticsCollector;
import org.finos.flowave.bpm.engine.impl.diagnostics.DiagnosticsRegistry;
import org.finos.flowave.bpm.engine.impl.digest.Default16ByteSaltGenerator;
import org.finos.flowave.bpm.engine.impl.digest.PasswordEncryptor;
import org.finos.flowave.bpm.engine.impl.digest.PasswordManager;
import org.finos.flowave.bpm.engine.impl.digest.SaltGenerator;
import org.finos.flowave.bpm.engine.impl.digest.Sha512HashDigest;
import org.finos.flowave.bpm.engine.impl.dmn.batch.DeleteHistoricDecisionInstancesJobHandler;
import org.finos.flowave.bpm.engine.impl.dmn.configuration.DmnEngineConfigurationBuilder;
import org.finos.flowave.bpm.engine.impl.dmn.deployer.DecisionDefinitionDeployer;
import org.finos.flowave.bpm.engine.impl.dmn.deployer.DecisionRequirementsDefinitionDeployer;
...
import org.finos.flowave.bpm.engine.impl.el.CommandContextFunctions;
import org.finos.flowave.bpm.engine.impl.el.DateTimeFunctions;
import org.finos.flowave.bpm.engine.impl.el.ElProviderCompatible;
import org.finos.flowave.bpm.engine.impl.el.ExpressionManager;
import org.finos.flowave.bpm.engine.impl.el.JuelExpressionManager;
import org.finos.flowave.bpm.engine.impl.errorcode.ExceptionCodeProvider;
import org.finos.flowave.bpm.engine.impl.event.CompensationEventHandler;
import org.finos.flowave.bpm.engine.impl.event.ConditionalEventHandler;
import org.finos.flowave.bpm.engine.impl.event.EventHandler;
import org.finos.flowave.bpm.engine.impl.event.EventHandlerImpl;
import org.finos.flowave.bpm.engine.impl.event.EventType;
import org.finos.flowave.bpm.engine.impl.event.SignalEventHandler;
import org.finos.flowave.bpm.engine.impl.externaltask.DefaultExternalTaskPriorityProvider;
import org.finos.flowave.bpm.engine.impl.form.deployer.CamundaFormDefinitionDeployer;
import org.finos.flowave.bpm.engine.impl.form.engine.FormEngine;
import org.finos.flowave.bpm.engine.impl.form.engine.HtmlFormEngine;
import org.finos.flowave.bpm.engine.impl.form.engine.JuelFormEngine;
import org.finos.flowave.bpm.engine.impl.form.entity.CamundaFormDefinitionManager;
import org.finos.flowave.bpm.engine.impl.form.type.AbstractFormFieldType;
import org.finos.flowave.bpm.engine.impl.form.type.BooleanFormType;
import org.finos.flowave.bpm.engine.impl.form.type.DateFormType;
import org.finos.flowave.bpm.engine.impl.form.type.FormTypes;
import org.finos.flowave.bpm.engine.impl.form.type.LongFormType;
import org.finos.flowave.bpm.engine.impl.form.type.StringFormType;
import org.finos.flowave.bpm.engine.impl.form.validator.FormFieldValidator;
import org.finos.flowave.bpm.engine.impl.form.validator.FormValidators;
import org.finos.flowave.bpm.engine.impl.form.validator.MaxLengthValidator;
import org.finos.flowave.bpm.engine.impl.form.validator.MaxValidator;
import org.finos.flowave.bpm.engine.impl.form.validator.MinLengthValidator;
import org.finos.flowave.bpm.engine.impl.form.validator.MinValidator;
import org.finos.flowave.bpm.engine.impl.form.validator.ReadOnlyValidator;
import org.finos.flowave.bpm.engine.impl.form.validator.RequiredValidator;
import org.finos.flowave.bpm.engine.impl.history.DefaultHistoryRemovalTimeProvider;
import org.finos.flowave.bpm.engine.impl.history.HistoryLevel;
import org.finos.flowave.bpm.engine.impl.history.HistoryRemovalTimeProvider;
import org.finos.flowave.bpm.engine.impl.history.event.HistoricDecisionInstanceManager;
import org.finos.flowave.bpm.engine.impl.history.event.HostnameProvider;
import org.finos.flowave.bpm.engine.impl.history.event.SimpleIpBasedProvider;
import org.finos.flowave.bpm.engine.impl.history.handler.CompositeDbHistoryEventHandler;
import org.finos.flowave.bpm.engine.impl.history.handler.CompositeHistoryEventHandler;
import org.finos.flowave.bpm.engine.impl.history.handler.DbHistoryEventHandler;
import org.finos.flowave.bpm.engine.impl.history.handler.HistoryEventHandler;
import org.finos.flowave.bpm.engine.impl.history.parser.HistoryParseListener;
import org.finos.flowave.bpm.engine.impl.history.producer.CacheAwareCmmnHistoryEventProducer;
import org.finos.flowave.bpm.engine.impl.history.producer.CacheAwareHistoryEventProducer;
import org.finos.flowave.bpm.engine.impl.history.producer.CmmnHistoryEventProducer;
import org.finos.flowave.bpm.engine.impl.history.producer.DefaultDmnHistoryEventProducer;
import org.finos.flowave.bpm.engine.impl.history.producer.DmnHistoryEventProducer;
import org.finos.flowave.bpm.engine.impl.history.producer.HistoryEventProducer;
import org.finos.flowave.bpm.engine.impl.history.transformer.CmmnHistoryTransformListener;
import org.finos.flowave.bpm.engine.impl.identity.DefaultPasswordPolicyImpl;
import org.finos.flowave.bpm.engine.impl.identity.ReadOnlyIdentityProvider;
import org.finos.flowave.bpm.engine.impl.identity.WritableIdentityProvider;
import org.finos.flowave.bpm.engine.impl.identity.db.DbIdentityServiceProvider;
import org.finos.flowave.bpm.engine.impl.incident.CompositeIncidentHandler;
import org.finos.flowave.bpm.engine.impl.incident.DefaultIncidentHandler;
import org.finos.flowave.bpm.engine.impl.incident.IncidentHandler;
import org.finos.flowave.bpm.engine.impl.interceptor.CommandContextFactory;
import org.finos.flowave.bpm.engine.impl.interceptor.CommandExecutor;
import org.finos.flowave.bpm.engine.impl.interceptor.CommandExecutorImpl;
import org.finos.flowave.bpm.engine.impl.interceptor.CommandInterceptor;
import org.finos.flowave.bpm.engine.impl.interceptor.DelegateInterceptor;
import org.finos.flowave.bpm.engine.impl.interceptor.ExceptionCodeInterceptor;
import org.finos.flowave.bpm.engine.impl.interceptor.SessionFactory;
import org.finos.flowave.bpm.engine.impl.jobexecutor.AsyncContinuationJobHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.DefaultFailedJobCommandFactory;
import org.finos.flowave.bpm.engine.impl.jobexecutor.DefaultJobExecutor;
import org.finos.flowave.bpm.engine.impl.jobexecutor.DefaultJobPriorityProvider;
import org.finos.flowave.bpm.engine.impl.jobexecutor.FailedJobCommandFactory;
import org.finos.flowave.bpm.engine.impl.jobexecutor.JobDeclaration;
import org.finos.flowave.bpm.engine.impl.jobexecutor.JobExecutor;
import org.finos.flowave.bpm.engine.impl.jobexecutor.JobHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.NotifyAcquisitionRejectedJobsHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.ProcessEventJobHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.RejectedJobsHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.TimerActivateJobDefinitionHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.TimerActivateProcessDefinitionHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.TimerCatchIntermediateEventJobHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.TimerExecuteNestedActivityJobHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.TimerStartEventJobHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.TimerStartEventSubprocessJobHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.TimerSuspendJobDefinitionHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.TimerSuspendProcessDefinitionHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.TimerTaskListenerJobHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.historycleanup.BatchWindowManager;
import org.finos.flowave.bpm.engine.impl.jobexecutor.historycleanup.DefaultBatchWindowManager;
import org.finos.flowave.bpm.engine.impl.jobexecutor.historycleanup.HistoryCleanupBatch;
import org.finos.flowave.bpm.engine.impl.jobexecutor.historycleanup.HistoryCleanupHandler;
import org.finos.flowave.bpm.engine.impl.jobexecutor.historycleanup.HistoryCleanupHelper;
import org.finos.flowave.bpm.engine.impl.jobexecutor.historycleanup.HistoryCleanupJobHandler;
import org.finos.flowave.bpm.engine.impl.metrics.MetricsRegistry;
import org.finos.flowave.bpm.engine.impl.metrics.MetricsReporterIdProvider;
import org.finos.flowave.bpm.engine.impl.metrics.parser.MetricsBpmnParseListener;
import org.finos.flowave.bpm.engine.impl.metrics.parser.MetricsCmmnTransformListener;
import org.finos.flowave.bpm.engine.impl.metrics.reporter.DbMetricsReporter;
import org.finos.flowave.bpm.engine.impl.migration.DefaultMigrationActivityMatcher;
import org.finos.flowave.bpm.engine.impl.migration.DefaultMigrationInstructionGenerator;
import org.finos.flowave.bpm.engine.impl.migration.MigrationActivityMatcher;
import org.finos.flowave.bpm.engine.impl.migration.MigrationInstructionGenerator;
import org.finos.flowave.bpm.engine.impl.migration.batch.MigrationBatchJobHandler;
import org.finos.flowave.bpm.engine.impl.migration.validation.activity.MigrationActivityValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.activity.NoCompensationHandlerActivityValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.activity.SupportedActivityValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.activity.SupportedPassiveEventTriggerActivityValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instance.AsyncAfterMigrationValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instance.AsyncMigrationValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instance.AsyncProcessStartMigrationValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instance.MigratingActivityInstanceValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instance.MigratingCompensationInstanceValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instance.MigratingTransitionInstanceValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instance.NoUnmappedCompensationStartEventValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instance.NoUnmappedLeafInstanceValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instance.SupportedActivityInstanceValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instance.VariableConflictActivityInstanceValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instruction.AdditionalFlowScopeInstructionValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instruction.CannotAddMultiInstanceBodyValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instruction.CannotAddMultiInstanceInnerActivityValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instruction.CannotRemoveMultiInstanceInnerActivityValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instruction.ConditionalEventUpdateEventTriggerValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instruction.GatewayMappingValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instruction.MigrationInstructionValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instruction.OnlyOnceMappedActivityInstructionValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instruction.SameBehaviorInstructionValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instruction.SameEventScopeInstructionValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instruction.SameEventTypeValidator;
import org.finos.flowave.bpm.engine.impl.migration.validation.instruction.UpdateEventTriggersValidator;
import org.finos.flowave.bpm.engine.impl.optimize.OptimizeManager;
import org.finos.flowave.bpm.engine.impl.persistence.GenericManagerFactory;
import org.finos.flowave.bpm.engine.impl.persistence.deploy.Deployer;
import org.finos.flowave.bpm.engine.impl.persistence.deploy.cache.CacheFactory;
import org.finos.flowave.bpm.engine.impl.persistence.deploy.cache.DefaultCacheFactory;
import org.finos.flowave.bpm.engine.impl.persistence.deploy.cache.DeploymentCache;
import org.finos.flowave.bpm.engine.impl.persistence.entity.AttachmentManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.AuthorizationManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.BatchManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.ByteArrayManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.CommentManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.DeploymentManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.EventSubscriptionManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.ExecutionManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.ExternalTaskManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.FilterManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.HistoricActivityInstanceManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.HistoricBatchManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.HistoricCaseActivityInstanceManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.HistoricCaseInstanceManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.HistoricDetailManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.HistoricExternalTaskLogManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.HistoricIdentityLinkLogManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.HistoricIncidentManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.HistoricJobLogManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.HistoricProcessInstanceManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.HistoricStatisticsManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.HistoricTaskInstanceManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.HistoricVariableInstanceManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.IdentityInfoManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.IdentityLinkManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.IncidentManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.JobDefinitionManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.JobManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.MeterLogManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.ProcessDefinitionManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.PropertyManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.ReportManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.ResourceManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.SchemaLogManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.StatisticsManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.TableDataManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.TaskManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.TaskReportManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.TenantManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.UserOperationLogManager;
import org.finos.flowave.bpm.engine.impl.persistence.entity.VariableInstanceManager;
import org.finos.flowave.bpm.engine.impl.repository.DefaultDeploymentHandlerFactory;
import org.finos.flowave.bpm.engine.impl.runtime.ConditionHandler;
import org.finos.flowave.bpm.engine.impl.runtime.CorrelationHandler;
import org.finos.flowave.bpm.engine.impl.runtime.DefaultConditionHandler;
import org.finos.flowave.bpm.engine.impl.runtime.DefaultCorrelationHandler;
import org.finos.flowave.bpm.engine.impl.runtime.DefaultDeserializationTypeValidator;
import org.finos.flowave.bpm.engine.impl.scripting.ScriptFactory;
import org.finos.flowave.bpm.engine.impl.scripting.engine.BeansResolverFactory;
import org.finos.flowave.bpm.engine.impl.scripting.engine.CamundaScriptEngineManager;
import org.finos.flowave.bpm.engine.impl.scripting.engine.DefaultScriptEngineResolver;
import org.finos.flowave.bpm.engine.impl.scripting.engine.ResolverFactory;
import org.finos.flowave.bpm.engine.impl.scripting.engine.ScriptBindingsFactory;
import org.finos.flowave.bpm.engine.impl.scripting.engine.ScriptEngineResolver;
import org.finos.flowave.bpm.engine.impl.scripting.engine.ScriptingEngines;
import org.finos.flowave.bpm.engine.impl.scripting.engine.VariableScopeResolverFactory;
import org.finos.flowave.bpm.engine.impl.scripting.env.ScriptEnvResolver;
import org.finos.flowave.bpm.engine.impl.scripting.env.ScriptingEnvironment;
import org.finos.flowave.bpm.engine.impl.telemetry.dto.DatabaseImpl;
import org.finos.flowave.bpm.engine.impl.telemetry.dto.InternalsImpl;
import org.finos.flowave.bpm.engine.impl.telemetry.dto.JdkImpl;
import org.finos.flowave.bpm.engine.impl.telemetry.dto.ProductImpl;
...
import org.finos.flowave.bpm.engine.impl.util.ParseUtil;
import org.finos.flowave.bpm.engine.impl.util.ProcessEngineDetails;
import org.finos.flowave.bpm.engine.impl.util.ReflectUtil;
import org.finos.flowave.bpm.engine.impl.variable.ValueTypeResolverImpl;
import org.finos.flowave.bpm.engine.impl.variable.serializer.BooleanValueSerializer;
import org.finos.flowave.bpm.engine.impl.variable.serializer.ByteArrayValueSerializer;
import org.finos.flowave.bpm.engine.impl.variable.serializer.DateValueSerializer;
import org.finos.flowave.bpm.engine.impl.variable.serializer.DefaultVariableSerializers;
import org.finos.flowave.bpm.engine.impl.variable.serializer.DoubleValueSerializer;
import org.finos.flowave.bpm.engine.impl.variable.serializer.FileValueSerializer;
import org.finos.flowave.bpm.engine.impl.variable.serializer.IntegerValueSerializer;
import org.finos.flowave.bpm.engine.impl.variable.serializer.JavaObjectSerializer;
import org.finos.flowave.bpm.engine.impl.variable.serializer.LongValueSerlializer;
import org.finos.flowave.bpm.engine.impl.variable.serializer.NullValueSerializer;
import org.finos.flowave.bpm.engine.impl.variable.serializer.ShortValueSerializer;
import org.finos.flowave.bpm.engine.impl.variable.serializer.StringValueSerializer;
import org.finos.flowave.bpm.engine.impl.variable.serializer.TypedValueSerializer;
import org.finos.flowave.bpm.engine.impl.variable.serializer.VariableSerializerFactory;
...
import org.finos.flowave.bpm.engine.repository.DecisionDefinition;
import org.finos.flowave.bpm.engine.repository.DecisionRequirementsDefinition;
import org.finos.flowave.bpm.engine.repository.DeploymentBuilder;
import org.finos.flowave.bpm.engine.repository.DeploymentHandlerFactory;
import org.finos.flowave.bpm.engine.runtime.Incident;
import org.finos.flowave.bpm.engine.runtime.WhitelistingDeserializationTypeValidator;
import org.finos.flowave.bpm.engine.task.TaskQuery;
import org.finos.flowave.bpm.engine.test.mock.MocksResolverFactory;
import org.finos.flowave.bpm.engine.variable.Variables;
...
public abstract class ProcessEngineConfigurationImpl extends ProcessEngineConfiguration {
  ...
  public static final String DEFAULT_WS_SYNC_FACTORY = "org.finos.flowave.bpm.engine.impl.webservice.CxfWebServiceClientFactory";

  public static final String DEFAULT_MYBATIS_MAPPING_FILE = "org/finos/flowave/bpm/engine/impl/mapping/mappings.xml";
  ...
  /**
   * As an instance of {@link org.finos.flowave.bpm.engine.impl.history.handler.CompositeHistoryEventHandler}
    ...
    synchronized (ProcessEngineConfigurationImpl.class) {
      ...
      if (sqlSessionFactory == null) {
        ...
        try {
          ...
          if (isUseSharedSqlSessionFactory) {
            properties.put("prefix", "${@org.finos.flowave.bpm.engine.impl.context.Context@getProcessEngineConfiguration().databaseTablePrefix}");
          } else {
