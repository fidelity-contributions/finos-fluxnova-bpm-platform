...
import org.finos.flowave.bpm.spring.boot.starter.rest.CamundaBpmRestInitializer;
import org.finos.flowave.bpm.spring.boot.starter.rest.CamundaJerseyResourceConfig;
...
public class CamundaBpmRunRestConfiguration {
  ...
  public FilterRegistrationBean<Filter> processEngineAuthenticationFilter(JerseyApplicationPath applicationPath) {
    ...
    if (properties.getAuthentication() == null || CamundaBpmRunAuthenticationProperties.DEFAULT_AUTH.equals(properties.getAuthentication())) {
      registration.addInitParameter("authentication-provider", "org.finos.flowave.bpm.engine.rest.security.auth.impl.HttpBasicAuthenticationProvider");
    }
    return registration;
  }
