...
public class FluxnovaBpmRunRestConfiguration {
  ...
  @Bean
  @ConditionalOnProperty(name = "enabled", havingValue = "true", prefix = FluxnovaBpmRunCorsProperty.PREFIX)
  public FilterRegistrationBean<Filter> corsFilter(JerseyApplicationPath applicationPath) {
    FilterRegistrationBean<Filter> registration = new FilterRegistrationBean<>();
    registration.setName("fluxnova-cors");
    ...
    registration.addInitParameter(CorsFilter.PARAM_CORS_ALLOWED_ORIGINS,
                                  fluxnovaBpmRunProperties.getCors().getAllowedOrigins());
    registration.addInitParameter(CorsFilter.PARAM_CORS_ALLOWED_METHODS,
                                  FluxnovaBpmRunCorsProperty.DEFAULT_HTTP_METHODS);
    registration.addInitParameter(CorsFilter.PARAM_CORS_ALLOWED_HEADERS,
                                  fluxnovaBpmRunProperties.getCors().getAllowedHeaders());
    registration.addInitParameter(CorsFilter.PARAM_CORS_EXPOSED_HEADERS,
                                  fluxnovaBpmRunProperties.getCors().getExposedHeaders());
    registration.addInitParameter(CorsFilter.PARAM_CORS_SUPPORT_CREDENTIALS,
                                  String.valueOf(fluxnovaBpmRunProperties.getCors().getAllowCredentials()));
    registration.addInitParameter(CorsFilter.PARAM_CORS_PREFLIGHT_MAXAGE,
                                  fluxnovaBpmRunProperties.getCors().getPreflightMaxAge());

    return registration;
  }

  @Bean
  public FluxnovaJerseyResourceConfig fluxnovaRunJerseyResourceConfig() {
    FluxnovaJerseyResourceConfig camundaJerseyResourceConfig = new FluxnovaJerseyResourceConfig();
    camundaJerseyResourceConfig.setProperties(Collections.singletonMap("jersey.config.server.wadl.disableWadl", fluxnovaBpmRunProperties.getRest().isDisableWadl()));
